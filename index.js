  ...
  if (path === "/api/rooms") {
    try {
      if (env.CHESS_DB) {
        ...
        const rows = await rdb.prepare("SELECT id, red, black, status, watchers, updated_at FROM rooms_registry WHERE updated_at > ? ORDER BY updated_at DESC LIMIT 50").bind(Date.now() - 12e5).all();
        return new Response(JSON.stringify({ rooms: rows.results || [] }), { headers: { ...corsHeaders, "Content-Type": "application/json" } });
      }
    } catch (e) {
    }
